...
class SATextField: UITextField {
    ...
    */
    ...
    /// state. Defaults to UIColor.grayColor().
    @IBInspectable var placeholderTextColorFocused: UIColor = UIColor.white {
        didSet {
            configurePlaceholder()
        }
    }
    ...
    var slideAnimationDuration: TimeInterval = 0.15
    ...
    */
    fileprivate func selfInit() {
        addTarget(self, action: #selector(UITextFieldDelegate.textFieldDidBeginEditing(_:)), for: UIControlEvents.editingDidBegin)
        addTarget(self, action: #selector(UITextFieldDelegate.textFieldDidEndEditing(_:)), for: UIControlEvents.editingDidEnd)
        configurePlaceholder()
    }
    ...
    */
    ...
    var slidingPlaceholderLeadingConstraint: NSLayoutConstraint! {
        if let c = constraints.filter({ (item) -> Bool in
            if let l = item.firstItem as? UILabel , l.tag == ViewIdentifiers.slidePlaceholderLabel.rawValue && item.secondAttribute == NSLayoutAttribute.leading {
                return true
            }
    ...
    var slidingPlaceholderCenterYConstraint: NSLayoutConstraint {
        if let c = constraints.filter({ (item) -> Bool in
            if let l = item.firstItem as? UILabel , l.tag == ViewIdentifiers.slidePlaceholderLabel.rawValue && item.secondAttribute == NSLayoutAttribute.centerY {
                return true
            }
        ...
        if let p = placeholderLabel {
            if slidesPlaceholder {
                if !isFirstResponder && (text == nil || text!.isEmpty) {
                    ...
                    if animated {
                        UIView.animate(withDuration: slideAnimationDuration, delay: 0.0, options: UIViewAnimationOptions(), animations: setForInner, completion: nil)
                    } else {
                        setForInner()
                    }
                    ...
                    if animated {
                        UIView.animate(withDuration: slideAnimationDuration, delay: 0.0, options: UIViewAnimationOptions(), animations: setForOuter, completion: nil)
                    } else {
                        setForOuter()
                    }
                }
